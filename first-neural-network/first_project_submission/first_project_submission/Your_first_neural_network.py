...
#         self.activation_function = sigmoid
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            #hidden_error_term = np.dot(np.dot(hidden_error,hidden_outputs),(1-hidden_outputs.T))
...
# 
# #### 请将你的答案填写在下方
# >模型预测结果与真实数据在12月11号到11月21号之间比较吻合。12月后半段时，虽然模型总是可以正确预测出因变量的**走势**，比如何时升高何时下降，但是总是在真实数据的峰值处出现较大误差。
# >造成的原因我不知道，一种猜测是训练集样本没有覆盖12月后半段出现的这种波动，或者说造成12月后半段振幅的影响因素时训练集从来没有见过的。
